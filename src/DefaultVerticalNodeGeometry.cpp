...
DefaultVerticalNodeGeometry::DefaultVerticalNodeGeometry(AbstractGraphModel &graphModel) :
		AbstractNodeGeometry(graphModel), _portSize(20), _portSpasing(10), _fontMetrics(QFont()), _boldFontMetrics(QFont()) {
	QFont f;
	f.setBold(true);
...
void DefaultVerticalNodeGeometry::recomputeSize(NodeId const nodeId) const {
	...
	unsigned int totalOutPortsWidth = nOutPorts > 0 ? outPortWidth * nOutPorts + _portSpasing * (nOutPorts - 1)
													: 0;
